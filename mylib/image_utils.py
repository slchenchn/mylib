...
def save_cv2_image_as_chinese_path(
    img, dst_path, is_bgr=False, auto_mkdir=True, **kargs
):
...
def save_image_by_cv2(
    img, dst_path, is_bgr=False, if_norm=True, auto_mkdir=True, **kargs
):
...
def pil_save_img(img: ndarray, path: str, auto_mkdir=True):
    if auto_mkdir:
...
def save_as_gif(
    images: list, filename: str, is_bgr=False, duration=700, auto_mkdir=True
):
